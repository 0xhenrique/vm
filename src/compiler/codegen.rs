...
impl Compiler {
    ...
    fn compile_module(&mut self, expr: &SourceExpr) -> Result<(), CompileError> {
    ...
    fn compile_export(&mut self, expr: &SourceExpr, module_name: &str) -> Result<(), CompileError> {
        let items = match &expr.expr {
            LispExpr::List(items) => items,
            _ => return Err(CompileError::new(
    ...
    fn compile_import(&mut self, expr: &SourceExpr) -> Result<(), CompileError> {
        let items = match &expr.expr {
            LispExpr::List(items) => items,
            _ => return Err(CompileError::new(
