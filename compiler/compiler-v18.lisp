; ============================================================================
; Lisp Self-Hosting Compiler - Version 18
; ============================================================================
; Advanced Pattern Matching for defun:
; - Cons/list patterns: (h . t), ((a b . c) d)
; - Multiple argument patterns: ((x y) body)
; - Wildcard patterns: _
; - Nested patterns: ((a (b c)) body)
;
; Enables powerful list processing and elegant code
...
(defconst OP-CAR 'car)
(defconst OP-CDR 'cdr)
(defconst OP-IS-LIST 'list?)
(defconst OP-CONS 'cons)
...
; Check if pattern is a cons pattern (list with dot or multiple elements)
(defun is-cons-pattern?
  ((pattern)
    (if (list? pattern)
      (if (== pattern '())
        false
...
; Extract all variable names from a pattern (flat list, in order)
; Used to build environment for pattern-matched expressions
(defun pattern-variables
  (('_) '())
  ((pattern)
    (if (symbol? pattern)
      (cons pattern '())
      (if (list? pattern)
        (if (== pattern '())
          '()
          (append-bytecode (pattern-variables (car pattern))
                           (pattern-variables (cdr pattern))))
        '()))))

; Count how many arguments are in a pattern list
(defun pattern-arity
  (('()) 0)
  (((p . rest)) (+ 1 (pattern-arity rest))))
...
; ============================================================================
; Pattern Matching for Defun - v18 with Cons Patterns
; ============================================================================

; Compile a single pattern (for single-argument functions in v18)
; Returns bytecode for testing + bytecode for binding + environment
(defun compile-single-pattern
  ((pattern)
    (if (is-wildcard? pattern)
      ; Wildcard: always matches, no binding, empty env
      (cons '() (cons '() '()))
      (if (symbol? pattern)
        ; Variable: always matches, bind arg0, env has variable
        (cons '() (cons '() (cons pattern '())))
        (if (== pattern '())
          ; Empty list literal: test equality
          (cons (cons (make-instr-2 OP-LOAD-ARG 0)
                     (cons (make-instr-2 OP-PUSH '())
                          (cons (make-instr-1 OP-EQ)
                               (cons (make-instr-2 OP-JMP-IF-FALSE 99) '()))))
               (cons '() '()))
          (if (list? pattern)
            ; Cons pattern: test IsList, extract car/cdr
            (compile-cons-pattern pattern)
            ; Number or other literal: test equality
            (cons (cons (make-instr-2 OP-LOAD-ARG 0)
...
; Compile cons pattern like (h . t) or (a b c)
; Returns: (test-bytecode . (binding-bytecode . env))
(defun compile-cons-pattern
  ((pattern)
    ; Test: LoadArg 0, IsList, JmpIfFalse
    (cons (cons (make-instr-2 OP-LOAD-ARG 0)
               (cons (make-instr-1 OP-IS-LIST)
                    (cons (make-instr-2 OP-JMP-IF-FALSE 99) '())))
          ; Bindings: extract variables from pattern
          (compile-cons-bindings pattern 0))))

; Generate bindings for cons pattern, returns: (binding-bytecode . env)
; arg-or-local: 0 means use LoadArg, >0 means use GetLocal
(defun compile-cons-bindings
  ((pattern arg-or-local)
    (if (is-wildcard? pattern)
      ; Wildcard: no binding
      (cons '() '())
      (if (symbol? pattern)
        ; Variable: extract this value
        (if (== arg-or-local 0)
          ; First level: use LoadArg 0
          (cons '() (cons pattern '()))
          ; Nested: use GetLocal (not implemented in v18)
          (cons '() (cons pattern '())))
        (if (== pattern '())
          ; Empty list: no binding
          (cons '() '())
          (if (list? pattern)
            ; Cons: extract car and cdr recursively
            (compile-cons-bindings-list pattern arg-or-local)
            ; Literal: no binding
            (cons '() '())))))))

; Helper for list patterns
(defun compile-cons-bindings-list
  ((pattern arg-or-local)
    ; For (h . t), extract h as car, t as cdr
    (cons (append-bytecode
            ; Generate: LoadArg 0, Car (for h)
            (cons (make-instr-2 OP-LOAD-ARG 0)
                 (cons (make-instr-1 OP-CAR) '()))
            ; Generate: LoadArg 0, Cdr (for t)
            (cons (make-instr-2 OP-LOAD-ARG 0)
                 (cons (make-instr-1 OP-CDR) '())))
          ; Build environment: extract variables from pattern
...
(defun compile-clauses-list (clauses)
  (if (== (cdr clauses) '())
    ; Last clause: no fallthrough needed
    (compile-last-clause (car clauses))
    ; Multiple clauses: compile with fallthrough
    (compile-clause-with-next (car clauses) (cdr clauses))))

; Compile the last clause (no pattern test needed if it's a variable)
(defun compile-last-clause
  ((clause)
    (cons (get-clause-patterns clause)
          (append-bytecode (cdr (compile-expr (get-clause-body clause)
                                              (pattern-variables (get-first-pattern (get-clause-patterns clause)))))
                           (cons (make-instr-1 OP-RET) '())))))

; Compile a clause with fallthrough to rest
(defun compile-clause-with-next (clause rest)
  (cons (get-first-pattern (get-clause-patterns clause))
        ; Compile pattern, body, and rest
        (compile-clause-body-and-rest
          (get-first-pattern (get-clause-patterns clause))
          (get-clause-body clause)
          rest)))

; Helper to compile clause body with pattern matching
(defun compile-clause-body-and-rest
  ((pattern body rest)
    ; Get pattern compilation result
    (cons pattern
          ; For now, use simple compilation
          (append-bytecode (cdr (compile-expr body (pattern-variables pattern)))
